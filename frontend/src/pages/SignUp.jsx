...
function SignUp() {
    ...
    const dispatch=useDispatch()
     ...
     const handleGoogleAuth=async () => {
        if(!auth){
          return setErr("Google Sign-Up is not configured. Please sign up with email and password or configure VITE_FIREBASE_APIKEY in frontend/.env.")
        }
        if(!mobile){
    ...
    return (
        <div className='min-h-screen w-full flex items-center justify-center p-4' style={{ backgroundColor: bgColor }}>
            <div className={`bg-white rounded-xl shadow-lg w-full max-w-md p-8 border-[1px] `} style={{
                ...
                <h1 className={`text-3xl font-bold mb-2 `} style={{ color: primaryColor }}>FoodConnect</h1>
